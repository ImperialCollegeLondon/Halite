...
import time
import random
from Halite_classes import Functional, Functional_field
import libspud
import csv
...
def get_xml_extension(input_file):
    if input_file.endswith('flml'):
        ...
        print("extension of this file not compatible with Halite (yet):", input_file)
        sys.exit(0)
...
def set_up_functional(Halite_options,nPhases_orig,nPhases,nTime,path,fwd_output_name):
    '''Fills the functional object with info from Halite_options and by calculating some other options'''
    functional = Functional()
    functional.time = Halite_options.functional.time
    sensors_list = Halite_options.functional.Sensors
    observations_list = Halite_options.functional.Observations
    nsensors = Halite_options.functional.nsensors
    sensors = np.zeros((nsensors,len(Halite_options.functional.Sensors[0])))
    
    if len(observations_list[0]) > 1: ##consider loooping over nsensors to check if any of the sensors has more than 1 input
         observations = np.zeros((nsensors,nTime))
    elif len(observations_list[0]) == 1:
          observations = np.zeros((nsensors,1)) 
    else:
        observations = []

    #To store the sensor locations and observations as arrays instead of lists
    for i in range(nsensors):
        sensors[i] = sensors_list[i]
        if len(observations_list[0]) > 0:
            observations[i] = observations_list[i]
    functional.Sensors = sensors
    functional.Observations = observations
    functional.nsensors = nsensors
    functional.type = Halite_options.functional.type
    for i in range (len(Halite_options.functional.Functional_field)):
        temp_field = Functional_field()
        temp_field.name = Halite_options.functional.Functional_field[i].name
        temp_field.field_type = Halite_options.functional.Functional_field[i].field_type
        temp_field.phase = Halite_options.functional.Functional_field[i].phase
    ...
    #location_of_interest = Halite_options.functional.location_of_interest
...
def find_node_nearest_to_point(functional,vtk_template_path):
    ...
    sensors = functional.Sensors
    nsensors = functional.nsensors
    nDim = sensors.shape[1] 
    jCoords = np.zeros((nsensors))
...
def calculate_DF_explicitly(Halite_options,directories,idir,fwd_output_name,jTime,nTime,iwindow,n_in_window,functional,nPhases_orig,nPhases,iPhase, vtk_checkpoint_file, meshname_base):
    #print "calculating DF explicitly"

    #print "nTime", nTime
    #print range(jTime,nTime)
    sensors = functional.Sensors
    observations = functional.Observations
    nsensors = functional.nsensors
    nfields = len(Halite_options.Field_To_study_list)

    # 3D ify point as paraview does this
    point = np.zeros((nsensors,3))
    for i in range(nsensors):
        if sensors.shape[1]==2:
        #if sensors[i, :].shape[0]==2: #to check for the size of every point instead of the entire array
            #print "3D ifying point"
            point_3d = np.zeros((1,3))
            point_3d[0,0:2] = sensors[i,:]
            point[i,:] = point_3d
        else:
            point[i,:] = sensors[i,:]
    ...
    if functional.type == "standard":
        ...
        W = calculate_weight_matrix(Halite_options, functional, vtk_checkpoint_file, meshname_base)
        sensor_nodes = find_node_nearest_to_point(functional, vtk_checkpoint_file)

        if Halite_options.functional.time == "end_time":
            ...
            if len(observations) == 0 and Halite_options.functional.square_integrand == True:
                DF = 0.5*(solution_at_point - solution_at_point_unpert)*(solution_at_point - solution_at_point_unpert)
            elif len(observations) == 0 and Halite_options.functional.square_integrand == False:
                DF = solution_at_point - solution_at_point_unpert
            elif len(observations) > 0:
                if observations.shape[1] > 1:
                    Residual_at_point = np.subtract(solution_at_point,np.reshape(observations[:,nTime-1],(nsensors,1)))
                    Residual_at_point_unpert = np.subtract(solution_at_point_unpert,np.reshape(observations[:,nTime-1],(nsensors,1)))
                else:
                    Residual_at_point = np.subtract(solution_at_point,observations)
                    Residual_at_point_unpert = np.subtract(solution_at_point_unpert,observations)

                # calculating the functional as the square of the integrand
                if Halite_options.functional.square_integrand == True:
                    #V_point and V_unpert are allocatables
                    V_point = np.zeros((nfields,nsensors))
                    V_unpert = np.zeros((nfields,nsensors))

                    for isen_nods in range(nsensors):
                        V_point[:,isen_nods] = Residual_at_point[isen_nods,iPhase]*W[:,int(sensor_nodes[isen_nods]),nTime-1]
                        V_unpert[:,isen_nods] = Residual_at_point_unpert[isen_nods,iPhase]*W[:,int(sensor_nodes[isen_nods]),nTime-1]
                    DF = 0.5*(np.matmul(V_point[0,:],Residual_at_point[:,iPhase])) - 0.5*(np.matmul(V_unpert[0,:],Residual_at_point_unpert[:,iPhase]))
                elif Halite_options.functional.square_integrand == False:
                    DF = np.sum(Residual_at_point[:,iPhase]) - np.sum(Residual_at_point_unpert[:,iPhase])
...
#            DF = (solution_field[indices]-solution_field_unpert[indices])
        elif Halite_options.functional.time == "all_time":
            ...
            for iTime in range(jTime,nTime):
                ...
                if len(observations) == 0  and Halite_options.functional.square_integrand == True:
                    DF = DF + 0.5*delta_t * (solution_at_point - solution_at_point_unpert)*(solution_at_point - solution_at_point_unpert)
                elif len(observations) == 0  and Halite_options.functional.square_integrand == False:
                    DF = DF + delta_t * (solution_at_point - solution_at_point_unpert)

                #print "DF contribution", delta_t * (solution_at_point - solution_at_point_unpert)
                elif len(observations) > 0:            
                    if observations.shape[1] > 1:
                        Residual_at_point = np.subtract(solution_at_point,np.reshape(observations[:,iTime],(nsensors,1)))
                        Residual_at_point_unpert = np.subtract(solution_at_point_unpert,np.reshape(observations[:,iTime],(nsensors,1)))
                    else:
                        Residual_at_point = np.subtract(solution_at_point,observations)
                        Residual_at_point_unpert = np.subtract(solution_at_point_unpert,observations)
                    ...
                    if Halite_options.functional.square_integrand == True:
                        ...
                        V_unpert = np.zeros((nfields,nsensors))
                        for isen_nods in range(nsensors):
                            V_point[:,isen_nods] = Residual_at_point[isen_nods,iPhase]*W[:,int(sensor_nodes[isen_nods]),iTime]
                            V_unpert[:,isen_nods] = Residual_at_point_unpert[isen_nods,iPhase]*W[:,int(sensor_nodes[isen_nods]),iTime]
                        value = 0.5*(np.matmul(V_point[0,:],Residual_at_point[:,iPhase])) - 0.5*(np.matmul(V_unpert[0,:],Residual_at_point_unpert[:,iPhase]))
                    elif Halite_options.functional.square_integrand == False:
                        value = np.sum(Residual_at_point[:,iPhase]) - np.sum(Residual_at_point_unpert[:,iPhase])
...
#            DF = DF + delta_t * ( np.linalg.norm(velocity) * temperature  - np.linalg.norm(velocity_unpert) * temperature_unpert  ) # temperature reference field cancels
    ...
    if Halite_options.functional.type == "geothermal_over_time":
...
def perturb_initial_condition(path, test_name, phases_to_perturb, nPhases, nPhases_orig, fieldname, fieldtype, output_path, sigma, ic_matrix, iPert, gs, nSmooth, nPert, vol_frac, G_init, use_G,random_seed,iwindow,avoid_perturbing_near_wells):
    ...
    # useful for comparing different sets of Halite simulations
    if random_seed>0:
...
def create_unperturbed_initial_conditions_vtu_and_run(Halite_options,cwd,field_name,modified_vol_frac,xml_ext,have_wells,phases_to_perturb):
    ...
    if Halite_options.time_window == 0:
         Halite_options.time_window = t_final - t0# ensuring one time window

    nwindow = np.rint( (t_final - t0) / Halite_options.time_window ) 
    nwindow = nwindow.astype(np.int)
    nwindow = max(nwindow, 1)#At least 1 time-window covering the whole time

    dump_period_timesteps = libspud.get_option('/io/dump_period_in_timesteps/constant') 

    n_in_window = np.rint( Halite_options.time_window / (dump_period_timesteps * dt) ) + 1
    n_in_window = n_in_window.astype(np.int)

    nTime = np.rint( (t_final - t0) / (dump_period_timesteps * dt) ) + 1
    ...
    if (Halite_options.time_window > 0):
        ...
        try:
            libspud.set_option('io/dump_period_in_timesteps/constant', dump_period_timesteps)
        except:
            pass
...
# do for all phases_to_perturb???????????????????????????????
    ...
    if Halite_options.Halite_operation == 'second_order_da':
        string = 'cp ../' + fwd_output_name +'_initial_condition.vtu ./'
        os.system(string)
    ...
    string = Halite_options.executable + " " + new_xml
...
def create_folder_to_run(Halite_options, cwd, have_wells, foldername):
    ...
    string = 'cp ../' + Halite_options.input_file + ' ./'
    os.system(string)
...
def run_forward_model(Halite_options, cwd, foldername):
...
def calculate_unperturbed_functional(Halite_options,fwd_output_name,nTime,functional,nPhases_orig,vtk_checkpoint_file, meshname_base, ITS):
    #To calculate unperturbed F.
    ...
    sensors = functional.Sensors
    observations = functional.Observations
    nsensors = sensors.shape[0] #nsensors = functional.nsensors can also be used
    nfields = len(Halite_options.Field_To_study_list)
    ...
    for i in range(nsensors):
        if sensors.shape[1]==2:
        ### if sensors[i, :].shape[0]==2: can be used to check for the size of every point instead of the entire array
            #print "3D ifying point"
            point_3d = np.zeros((1,3))
            point_3d[0,0:2] = sensors[i,:]
            point[i,:] = point_3d
    ...
    if Halite_options.functional.time == "end_time":
        ...
        #To account for varying observations in time
        ...
        if Halite_options.functional.square_integrand == True:
            #V_unpert is an allocatable
            V_unpert = np.zeros((nfields,nsensors))
            for isen_nods in range(nsensors):
                V_unpert[:,isen_nods] = Residual_at_point_unpert[isen_nods,iPhase]*W[:,int(sensor_nodes[isen_nods]),nTime-1]
            F = 0.5*(np.matmul(V_unpert[0,:],Residual_at_point_unpert[:,iPhase]))
        else:
            F = np.sum(Residual_at_point_unpert[:,iPhase])

    if Halite_options.functional.time == "all_time":
        # get delta_t
        libspud.load_options(Halite_options.input_file)
        dt = libspud.get_option('/timestepping/timestep')
        libspud.clear_options()

        solutions_unpert_point = np.zeros((nsensors, nTime)) #for storing the solutions at sensors over time
        ...
        for iTime in range(nTime):
            ...
            probe = VTU_Probe(vtu_data.ugrid, point)
            solution_at_point_unpert = probe.GetField(f_name)
            solutions_unpert_point[:,iTime] = solution_at_point_unpert[:,0]
            ...
            if Halite_options.functional.square_integrand == True:
                ...
                for isen_nods in range(nsensors):
                    V_unpert[:,isen_nods] = Residual_at_point_unpert[isen_nods,iPhase]*W[:,int(sensor_nodes[isen_nods]),iTime]
                F_val = 0.5*(np.matmul(V_unpert[0,:],Residual_at_point_unpert[:,iPhase]))
            else:
                F_val= np.sum(Residual_at_point_unpert[:,iPhase])
            F = F + delta_t*F_val
    #To write solution at point to a csv file. Only works for functional calculated over all time
    with open('solutions_at_points_unpert'+str(ITS)+'.csv', 'w') as csvfile:
        csvwriter = csv.writer(csvfile)
        for col1 in solutions_unpert_point:
            csvwriter.writerow(col1)

    return F


def calculate_weight_matrix(Halite_options, functional, vtk_checkpoint_file, meshname_base):

    NONODS = get_nNodes_from_gmsh_file(meshname_base + '.msh')
    NSCALAR = len(Halite_options.Field_To_study_list)
    libspud.load_options(Halite_options.input_file)
    t0 = libspud.get_option('/timestepping/current_time')
    dt = libspud.get_option('/timestepping/timestep')
    t_final = libspud.get_option('/timestepping/finish_time')
    ...
    if Halite_options.functional.time == "all_time":
        for isen_nods in sensor_nodes:
            W[:,int(isen_nods),:] = 1#dt/DT
    #To compute the weight matrix for the functional evaluated at the end of time
    elif Halite_options.functional.time == "end_time":
        for isen_nods in sensor_nodes:
            W[:,int(isen_nods),nTime-1] = 1#dt/DT
    return W
